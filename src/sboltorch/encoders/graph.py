...
Each object's neighborhood becomes a graph whose nodes carry a (sbol_class, role)
...
class GraphEncoder:
    ...
    def _class_id(self, sbol_class: str | None) -> int:
        return self._class_ids.get(local_name(sbol_class or ""), 0)
    ...
    def encode(self, obj: SbolObject) -> Data:
        graph = obj.neighbors
        ...
        x = torch.tensor(
            [
                [self._class_id(n.sbol_class), self._role_id(role_of.get(n.iri)), self._name_id(n.display_id)]
