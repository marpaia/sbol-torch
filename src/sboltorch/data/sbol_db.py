...
class SbolDbClient:
    ...
    ``sbol_class`` / ``role`` / ``document_id`` filters, resolving the
    ...
    def fingerprint(self) -> str:
        ...
        """
        h = hashlib.sha256()
        for part in (self.base_url, self.sbol_class, self.role, self.document_id, self.label_key):
